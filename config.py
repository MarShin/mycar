...
DEFAULT_MODEL_TYPE = (
    "linear"  # (linear|categorical|rnn|imu|behavior|3d|localizer|latent)
)
...
USE_EARLY_STOP = (
    True  # would you like to stop the training if we see it's not improving fit?
)
EARLY_STOP_PATIENCE = 5  # how many epochs to wait before no improvement
MIN_DELTA = (
    0.0005  # early stop will want this much loss change before calling it improved.
)
PRINT_MODEL_SUMMARY = True  # print layers and weights to stdout
OPTIMIZER = None  # adam, sgd, rmsprop, etc.. None accepts default
LEARNING_RATE = 0.001  # only used when OPTIMIZER specified
LEARNING_RATE_DECAY = 0.0  # only used when OPTIMIZER specified
SEND_BEST_MODEL_TO_PI = (
    False  # change to true to automatically send best model during training
)
...
PRUNE_VAL_LOSS_DEGRADATION_LIMIT = (
    0.2  # The max amout of validation loss that is permitted during pruning.
)
PRUNE_EVAL_PERCENT_OF_DATASET = (
    0.05  # percent of dataset used to perform evaluation of model.
)
...
NUM_LAST_LAYERS_TO_TRAIN = (
    7  # when freezing layers, how many layers from the last should be allowed to train?
)
...
USE_NETWORKED_JS = (
    False  # should we listen for remote joystick control over the network?
)
NETWORK_JS_SERVER_IP = "192.168.0.1"  # when listening for network joystick control, which ip is serving this information
JOYSTICK_DEADZONE = (
    0.0  # when non zero, this is the smallest throttle before recording triggered.
)
JOYSTICK_THROTTLE_DIR = (
    -1.0
)  # use -1.0 to flip forward/backward, use 1.0 to use joystick's natural forward/backward
USE_FPV = False  # send camera data to FPV webserver
JOYSTICK_DEVICE_FILE = (
    "/dev/input/js0"  # this is the unix file use to access the joystick.
)
...
# RNN or 3D
SEQUENCE_LENGTH = (
    3  # some models use a number of images over time. This controls how many.
)

# IMU
HAVE_IMU = (
    False  # when true, this add a Mpu6050 part and records the data. Can be used with a
)
...
REC_COUNT_ALERT_CYC = (
    15  # how many cycles of 1/20 of a second to blink per REC_COUNT_ALERT records
)
...
SIM_HOST = (
    "127.0.0.1"  # when racing on virtual-race-league use host "trainmydonkey.com"
)
