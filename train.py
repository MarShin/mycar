...
def train(cfg, tub_names, model_name, transfer_model, model_type, continuous, aug):
    ...
    def generator(
        save_best, opts, data, batch_size, isTrainSet=True, min_records_to_train=1000
    ):
        ...
        while True:
            ...
            for key in keys:
                ...
                if len(batch_data) == batch_size:
                    ...
                    img_arr = np.array(inputs_img).reshape(
    ...
    if cfg.SHOW_PLOT:
        try:
            if do_plot:
                ...
                plt.savefig(
                    model_path + "_loss_acc_%f.%s" % (save_best.best, figure_format)
                )
                plt.show()
            else:
...
def sequence_train(
    cfg, tub_names, model_name, transfer_model, model_type, continuous, aug
):
    """
    use the specified data in tub_names to train an artifical neural network
    saves the output trained model as model_name
    ...
    train_data, val_data = train_test_split(
        sequences, test_size=(1 - cfg.TRAIN_TEST_SPLIT)
    )
    ...
    def generator(data, opt, batch_size=cfg.BATCH_SIZE):
        ...
        while True:
            ...
            for offset in range(0, num_records, batch_size):
                ...
                if look_ahead:
                    X = [
                        np.array(b_inputs_img).reshape(
                            batch_size, cfg.TARGET_H, cfg.TARGET_W, cfg.SEQUENCE_LENGTH
                        )
                    ]
...
def extract_data_from_pickles(cfg, tubs):
    ...
    for tub_path in t_paths:
        ...
        print(
            "found {} pickles writing json records and images in tub {}".format(
